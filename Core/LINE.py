...
class LINE(threading.Thread):
	...
	def __init__(self, api_token, secret, app='LINE', path='/callback', name='callback'):
		...
		key = self.get_key(MessageEvent, message=TextMessage)
		self.handler._handlers[key] = self.got_text
			...
			if cmd in self.command_call:
				if type(self.command_call[cmd]['call']) is str:
					...
					if ' ' in event.message.text:
						...
						del args[0]
						text = self.command_call[cmd]['call'](event, args=args)
					else:
						text = self.command_call[cmd]['call'](event, args=[])
				else:
					text = self.command_call[cmd]['call'](event)
			else:
				text = self.unknown_command
		else:
			if self.text_message is not None:
				text = self.text_message(event)
			else:
				text = "Umm... This bot no reply feature."
		if text is not None and type(text) in (str, unicode):
